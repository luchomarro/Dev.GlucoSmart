...
class RecomendacionesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (perfil.imc > 25.0 && perfil.imc < 30.0) {
      nivelRiesgo += 1;
      alertas.add("Sobrepeso detectado (IMC: ${perfil.imc.toStringAsFixed(1)}).");
    } else if (perfil.imc >= 30.0) {
      nivelRiesgo += 2;
      alertas.add("Obesidad detectada (IMC: ${perfil.imc.toStringAsFixed(1)}). El peso es un factor clave en la resistencia a la insulina.");
    }

    // Factor B: Condiciones genéticas y de estilo de vida
    if (perfil.condicionesMedicas.contains('Familiares con diabetes')) {
      nivelRiesgo += 1;
      alertas.add("Antecedentes genéticos incrementan la predisposición.");
    }
    if (perfil.condicionesMedicas.contains('Sedentarismo')) {
      nivelRiesgo += 1;
      sugerencias.add("Inicia rutinas de caminata de 30 minutos diarios.");
    }
    ...
    if (regGlucosa.isNotEmpty) {
      ...
      if (promGlucosa >= 100 && promGlucosa < 125) {
        nivelRiesgo += 1;
        alertas.add("Tu promedio de glucosa reciente (${promGlucosa.toStringAsFixed(0)} mg/dL) sugiere posible Prediabetes.");
      } else if (promGlucosa >= 125) {
        nivelRiesgo += 2;
        alertas.add("Promedio de glucosa alto (${promGlucosa.toStringAsFixed(0)} mg/dL). Consulta a un médico inmediatamente.");
      }
    }
    ...
    if (nivelRiesgo == 0) {
      tituloDiagnostico = "Riesgo Bajo de Diabetes";
      colorDiagnostico = const Color(0xFF1DB954);
      sugerencias.add("Mantén tu excelente estilo de vida y dieta balanceada.");
    } else if (nivelRiesgo == 1) {
      tituloDiagnostico = "Riesgo Moderado / Prediabetes";
      colorDiagnostico = const Color(0xFFF59E0B); // Naranja
      iconoDiagnostico = Icons.warning_amber_rounded;
      sugerencias.add("Reduce el consumo de azúcares procesados.");
      sugerencias.add("Aumenta tu actividad física semanal a 150 minutos.");
    } else {
      tituloDiagnostico = "Riesgo Alto Detectado";
      colorDiagnostico = const Color(0xFFFF3B30); // Rojo
      iconoDiagnostico = Icons.error_outline;
      sugerencias.add("Se recomienda un examen HbA1c (Hemoglobina Glicosilada) de laboratorio.");
      sugerencias.add("Consulta con un endocrinólogo esta misma semana.");
    }

    return Scaffold(
      backgroundColor: const Color(0xFFF4F8FF),
      appBar: AppBar(
        title: const Text("Predicción IA", style: TextStyle(fontWeight: FontWeight.w800, color: Color(0xFF12305B))),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text("Factores Clínicos Detectados", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF12305B))),
            ...
            Container(
              ...
              child: alertas.isEmpty
                  ? const Text("No se detectaron factores de riesgo significativos en tu perfil.", style: TextStyle(color: Color(0xFF6B7280)))
                  : Column(
                children: alertas.map((alerta) => Padding(
                  ...
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Icon(Icons.analytics_outlined, color: Color(0xFF2F80ED), size: 20),
            ...
            const Text("Plan de Acción Generado por IA", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF12305B))),
            const SizedBox(height: 12),
            Container(
              width: double.infinity, padding: const EdgeInsets.all(20),
